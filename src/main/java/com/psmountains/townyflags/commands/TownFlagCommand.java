...
public class TownFlagCommand implements CommandExecutor, TabCompleter {
	...
	public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
		...
		if (args.length < 1) {
			p.sendMessage("§cUsage: /t flag <set|remove|info|reload>");
			return true;
		}
		...
		switch (args[0].toLowerCase()) {
			case "set" -> {
				...
				if (!isAllowed(p, "flags.town-allowed-roles", "town")) {
			...
			default -> {
				p.sendMessage("§cUnknown flag subcommand. Use /t flag set/remove/info/reload");
				return true;
			}
		}
	}
	...
	public List<String> onTabComplete(CommandSender sender, Command command, String alias, String[] args) {
		if (args.length == 1) {
			return List.of("set", "remove", "info", "list");
		}
